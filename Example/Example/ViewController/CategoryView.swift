...
class CategoryView: UIView {
    ...
    private var currentCategory: PhotoLibraryClient.Category = .all {
        didSet {
            for view in stackView.arrangedSubviews {
                guard let button = view as? UIButton,
                      let identifier = button.accessibilityIdentifier,
                      let category = PhotoLibraryClient.Category(rawValue: identifier)
                else { continue }
    ...
    private func setupViews() {
        ...
        for category in PhotoLibraryClient.Category.allCases {
            let button = UIButton()
    ...
    @objc private func categoryButtonTapped(_ sender: UIButton) {
        guard let category = PhotoLibraryClient.Category(rawValue: sender.accessibilityIdentifier ?? "") else { return }
